...
m <- as.matrix(dtm)   
dim(m)   
write.csv(m, file="WordCloud Lab/dtm.csv")   
...
mt <- as.matrix(tdm)   
dim(mt)   
write.csv(mt, file="WordCloud Lab/tdm.csv") 
...
p <- p + geom_bar(stat="identity")    #stat == identity will skip the aggregation and y values will be provided. 
p <- p + theme(axis.text.x=element_text(angle=45, hjust=1))   
p 

ggsave("WordCloud Lab/Words_freq_gt_250.png", p)
